...
/**
 * Proxy to FastAPI backend: GET /verify/candles (HTML page to verify 1000 candles bootstrap).
 */
export async function GET(_request: NextRequest) {
  const backendUrl = getBackendUrl();
  if (!backendUrl) {
    return new NextResponse(
      `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Backend not configured</title></head><body style="font-family:system-ui;background:#0f0f0f;color:#e0e0e0;padding:20px;"><h1>Backend not configured</h1><p>Set <code>BACKEND_URL</code> in .env, or run the backend:</p><pre style="background:#000;padding:12px;border-radius:8px;">cd backend && USE_MEMORY_STORE=1 python3 -m uvicorn app.main:app --host 127.0.0.1 --port 8000</pre><p>Then open <a href="/api/backend/verify/candles" style="color:#26a69a;">/api/backend/verify/candles</a> again.</p></body></html>`,
      { status: 503, headers: { 'Content-Type': 'text/html; charset=utf-8' } }
    );
  }

  try {
    const res = await fetch(`${backendUrl}/verify/candles`, {
    ...
    console.error('Backend verify/candles proxy error:', err);
    return new NextResponse(
      `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Backend not reachable</title></head><body style="font-family:system-ui;background:#0f0f0f;color:#e0e0e0;padding:20px;"><h1>Backend not reachable</h1><p>Start the backend (from project root):</p><pre style="background:#000;padding:12px;border-radius:8px;">cd backend && USE_MEMORY_STORE=1 python3 -m uvicorn app.main:app --host 127.0.0.1 --port 8000</pre><p>Wait for "Binance WebSocket task started", then <a href="/api/backend/verify/candles" style="color:#26a69a;">reload</a>.</p></body></html>`,
